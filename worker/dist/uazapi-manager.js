    ...
    try {
        ...
        if (phone) {
            return {
                state: 'connected',
                phone,
                nome: data?.instance?.profileName ?? data?.instance?.name ?? null,
                qr: null,
            };
        }
...
async function checarInstancia(token) {
    try {
...
async function adminApi(method, path, body) {
    ...
    if (res.status === 429)
        throw new UazapiRateLimitError(`uazapi admin ${method} ${path} → 429`);
...
async function instanceApi(instanceToken, method, path, body) {
    ...
    if (res.status === 429)
        throw new UazapiRateLimitError(`uazapi inst ${method} ${path} → 429`);
    const text = await res.text();
    if (!res.ok)
...
export class UazapiManager {
    ...
    rateLimitados = new Set(); // contaIds atualmente sinalizadas como rate-limited
            ...
            if (inst.status === 'connected') {
                ...
                try {
                    ...
                    await this.supabase.from('conexoes').upsert({ conta_id: contaId, status: 'conectado', qr_code: null, comando: null,
                        ...
                        uazapi_instance_token: inst.token }, { onConflict: 'conta_id' });
                    this.rateLimitados.delete(contaId);
                }
                catch (err) {
                    logger.warn({ contaId, err }, 'uazapi: restaurarSessoes — falha ao sincronizar DB');
                }
    ...
    async conectar(contaId) {
        ...
        await this.supabase.from('conexoes').upsert({ conta_id: contaId, status: 'conectando', qr_code: null, comando: null,
            uazapi_instance_token: token, rate_limitado: false }, { onConflict: 'conta_id' });
    ...
    async desconectar(contaId) {
        ...
        await this.supabase.from('conexoes').upsert({ conta_id: contaId, status: 'desconectado', qr_code: null, comando: null,
            numero_conectado: null, device_name: null, rate_limitado: false }, { onConflict: 'conta_id' });
    }
    ...
    async reconectar(contaId) {
        ...
        let estado = { state: 'disconnected', phone: null, nome: null, qr: null };
        try {
            estado = await this.pegarEstado(contaId);
        }
        catch (err) {
            logger.warn({ contaId, err }, 'uazapi: reconectar — falha ao checar estado');
        }
        if (estado.state === 'connected') {
            ...
            if (token) {
                try {
                    ...
                    logger.warn({ contaId, err }, 'uazapi: reconectar — falha ao sincronizar DB');
                }
            }
            this.connected.add(contaId);
            this.iniciarPolling(contaId);
        ...
        if (estado.rateLimited) {
            ...
            try {
                ...
                logger.warn({ contaId, err }, 'uazapi: reconectar — falha ao limpar comando');
            }
            return;
        }
    ...
    async buscarEGravarQR(contaId) {
        ...
        try {
            ...
            if (info.qr) {
                await this.supabase.from('conexoes').upsert({ conta_id: contaId, qr_code: info.qr, status: 'conectando', rate_limitado: false }, { onConflict: 'conta_id' });
                this.rateLimitados.delete(contaId);
    ...
    async marcarDesconectado(contaId) {
        await this.supabase.from('conexoes').upsert({ conta_id: contaId, status: 'desconectado', qr_code: null, comando: null, rate_limitado: false }, { onConflict: 'conta_id' });
        this.rateLimitados.delete(contaId);
    }
    ...
    async sinalizarRateLimit(contaId) {
        if (this.rateLimitados.has(contaId))
            return;
        ...
        try {
            ...
            logger.warn({ contaId, err }, 'uazapi: falha ao sinalizar rate limit');
        }
    }
    // Limpa a sinalização de rate limit assim que uma checagem volta a funcionar.
    async limparRateLimit(contaId) {
        if (!this.rateLimitados.has(contaId))
            return;
        this.rateLimitados.delete(contaId);
        try {
            ...
            logger.warn({ contaId, err }, 'uazapi: falha ao limpar rate limit');
        }
    }
        ...
        for (const row of conexoes) {
            ...
            if (inst.status === 'connected') {
                ...
                try {
                    ...
                    logger.warn({ contaId, err }, 'uazapi: sincronizarConexoes — falha ao sincronizar DB');
                }
                this.iniciarPolling(contaId);
                ...
                await this.buscarEGravarQR(contaId);
                try {
                    await this.supabase.from('conexoes').upsert({ conta_id: contaId, status: 'conectando', comando: null }, { onConflict: 'conta_id' });
                }
                catch (err) {
                    logger.warn({ contaId, err }, 'uazapi: sincronizarConexoes — falha ao atualizar status conectando');
                }
                this.iniciarPolling(contaId);
    ...
    async tentarReconectarAuto(contaId) {
        ...
        for (let i = 1; i <= MAX_TENTATIVAS; i++) {
            if (!this.polling.has(contaId))
                return 'falhou'; // desconectado manualmente durante tentativa
            const token = this.instanceTokens.get(contaId);
            if (!token)
                return 'falhou';
            ...
            try {
                ...
                logger.warn({ contaId, tentativa: i, err }, 'uazapi: tentarReconectarAuto — falha ao checar estado');
            }
            if (estado.state === 'connected') {
                this.connected.add(contaId);
                try {
                    ...
                    logger.warn({ contaId, err }, 'uazapi: tentarReconectarAuto — falha ao sincronizar DB pós-reconexão');
                }
            ...
            if (estado.state === 'connecting') {
                ...
                await this.buscarEGravarQR(contaId);
                try {
                    await this.supabase.from('conexoes').upsert({ conta_id: contaId, status: 'conectando', comando: null, rate_limitado: false }, { onConflict: 'conta_id' });
                    this.rateLimitados.delete(contaId);
                }
                catch (err) {
                    logger.warn({ contaId, err }, 'uazapi: tentarReconectarAuto — falha ao atualizar status conectando');
                }
        ...
        const loop = async () => {
            while (this.polling.has(contaId)) {
                ...
                try {
                    ...
                    const info = tok ? await verificarEstado(tok) : { state: 'disconnected', phone: null, nome: null, qr: null };
                    ...
                    if (state === 'connected' && !this.connected.has(contaId)) {
                        ...
                        try {
                            ...
                            logger.warn({ contaId, err }, 'uazapi: polling — falha ao sincronizar DB pós-conexão');
                        }
                    }
                    if (state !== 'connected' && this.connected.has(contaId)) {
                        this.connected.delete(contaId);
                    ...
                    if (erros >= MAX_ERROS) {
                        ...
                        try {
                            ...
                            logger.warn({ contaId, err }, 'uazapi: polling — falha ao marcar desconectado');
                        }
                        return;
                    }
                }
