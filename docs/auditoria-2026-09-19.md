...
| **OPS-N3** | ✅ **RESOLVIDO (2026-09-19)** | **CRÍTICO** | **`UAZAPI_ADMIN_TOKEN` estava vazio em produção** (confirmado com `vercel env pull` + teste direto contra a API da uazapi: `401 {"error":"Missing AdminToken Header"}`). Esse token só serve para a API de ADMINISTRAÇÃO da uazapi (`/instance/all` — listar/gerenciar instâncias); quem autentica o ENVIO em si é o token da própria instância, já salvo em `conexoes.uazapi_instance_token`. Consequência: toda checagem que passava por `getAllInstances()` — inclusive a correção do OPS-N2 momentos antes — falhava/retornava vazio, então "Forçar" continuava reportando "WhatsApp desconectado" mesmo com o WhatsApp genuinamente conectado (os lembretes automáticos do cron não eram afetados, porque o envio de verdade usa o token salvo no banco, não a API de administração — só a sincronização de status silenciosa, `sincronizarConexoes`, dependia do token e vinha falhando calada, presa no catch). **Correção definitiva:** `forcarEnvioAction` e `enviarRespostaAction` (resposta manual em /atendimento) passaram a ler `conexoes.status`/`uazapi_instance_token` direto do banco — a MESMA fonte que o cron usa para decidir quem está elegível — em vez de fazer uma checagem ao vivo contra a API de administração. Isso elimina a dependência do admin token do caminho crítico de envio inteiro. **Pendência sua:** o admin token continua necessário para `sincronizarConexoes` (o que mantém `conexoes.status` e o banner "WhatsApp desconectado" da Dashboard atualizados automaticamente) — pegar o token real no painel da uazapi/supercloudstore e configurar `UAZAPI_ADMIN_TOKEN` na Vercel (Production) | `log/_actions/log.ts`, `atendimento/_actions/mensagens.ts` |
