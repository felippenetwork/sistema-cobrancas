...
- **Papel dentro da conta (`admin` × `atendente`) é imposto no BANCO, não só na interface.** Achado aberto (SEG-N4, auditoria 2026-09-19): `conta_do_usuario()` (migration 0020) inclui `membros_conta`, e a policy de `configuracoes` usa essa função — um atendente lê pelo client anon as credenciais da conta (Meta, EfiBank incl. certificado, LookDefense) e pode trocá-las (desviando PIX). Toda tabela com segredo ou configuração sensível precisa de policy específica de dono/admin; ação que o atendente precisa (ex.: responder cliente, que usa o token Meta) roda no servidor com service role depois de verificar a associação — nunca lendo o segredo com o client do usuário. Ver `docs/auditoria-2026-09-19.md`.
