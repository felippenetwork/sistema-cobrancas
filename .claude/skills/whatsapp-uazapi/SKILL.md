...
description: Arquitetura real de conexão e disparo WhatsApp deste projeto — uazapi direto (Vercel ↔ uazapi via cron externo), único canal do produto (Meta Cloud API foi removida em 2026-09-21), SEM worker/VPS. Use SEMPRE que mexer em conexão WhatsApp, pareamento/QR, envio de mensagem, ritmo/fila de disparo, ou campanhas em `/disparos`. Define o que roda onde hoje, o ritmo anti-ban e por que o diretório `worker/` é código morto. Consulte antes de escrever qualquer código que conecte ou envie pelo WhatsApp — o erro mais caro aqui é reintroduzir a arquitetura antiga (Baileys + worker na Vortexus, ou a Meta Cloud API), ambas abandonadas.
---
...
- **Duas credenciais uazapi diferentes, não confundir.** `UAZAPI_ADMIN_TOKEN` (env var, uma só para toda a plataforma) autentica só a API de ADMINISTRAÇÃO (`/instance/all` — listar/criar/gerenciar instâncias). Quem autentica o ENVIO em si (`/send/text`, `/send/presence`) é o token DA PRÓPRIA INSTÂNCIA, gerado quando ela conecta e salvo em `conexoes.uazapi_instance_token` — nunca precisa do admin token. **Qualquer código que decide "posso enviar para esta conta" deve ler `conexoes.status`/`uazapi_instance_token` do banco (a mesma fonte que o cron e a tela `/conexao` usam), nunca fazer uma checagem ao vivo contra `/instance/all`** — isso criaria uma dependência desnecessária do admin token no caminho crítico de envio. Achado real (2026-09-19, OPS-N3): a checagem via `getAllInstances()` estava falhando em produção e isso fazia "Forçar" reportar "desconectado" com o WhatsApp genuinamente conectado — os lembretes automáticos do cron não foram afetados porque o envio de verdade nunca dependeu desse token, só a sincronização de status (`sincronizarConexoes`) dependia. **Cuidado ao diagnosticar isso de novo:** `vercel env pull` esconde (redige) o valor de variáveis marcadas "Sensitive" mesmo quando preenchidas — um valor vazio no pull NÃO prova que a variável está vazia de verdade em produção; conferir direto no painel do provedor (uazapi/supercloudstore) ou via um teste de comportamento, nunca só pelo `env pull`. O admin token continua necessário só para essa sincronização automática (`sincronizarConexoes`, cron `whatsapp-uazapi`) — sem ele, `conexoes.status` fica congelado no último valor conhecido e o banner "WhatsApp desconectado" da Dashboard para de refletir desconexões reais.
- **Não reimplemente a checagem de instância/envio uazapi fora de `lib/uazapi.ts`.** Achado real (2026-09-19, OPS-N2): `forcarEnvioAction` (Log → "Forçar") tinha sua própria cópia de `/instance/all` + `/send/text` via `fetch` cru, e tratava QUALQUER falha nessa checagem (rede, HTTP 429 de rate limit) como "instância não encontrada" — reportando "WhatsApp desconectado" mesmo com a conexão ok. Use sempre `getAllInstances`, `instName`, `sendText` e `UazapiRateLimitError` de `lib/uazapi.ts` — é a única fonte de verdade que já trata 429 corretamente e normaliza o número antes de enviar.
...
- **Meta Cloud API removida do produto (decisão do dono, 2026-09-21).** uazapi é hoje o ÚNICO canal WhatsApp — não existe mais `app/api/cron/whatsapp/route.ts`, `/api/webhooks/whatsapp` não tem mais o branch Meta, `/api/meta/*` foi apagado, e `configuracoes`/`notificacoes_config` não têm mais colunas `meta_*` (migration `0034_remove_meta_cloud_api.sql`). `/wa-templates` continua existindo, mas mudou de propósito: hoje é só um CRUD simples de texto livre para os modelos usados em `/disparos` (sem processo de aprovação, sem categoria/idioma Meta). Se encontrar menção a "Meta", "WhatsApp Business API" ou `graph.facebook.com` em código, é **achado de algo esquecido na remoção** — não é arquitetura válida, corrigir.
- **Código morto:** `worker/src/*` (incluindo `uazapi-manager.ts` e `whatsapp-worker.ts`) continua no repositório mas **não roda em lugar nenhum** — inclusive ainda tem código e colunas `meta_*` do worker antigo, sem efeito algum. Não editar ali esperando efeito em produção; qualquer mudança de comportamento de envio/lembrete/campanha se faz em `app/api/cron/whatsapp-uazapi/route.ts`. Se um dia o worker for oficialmente reativado ou removido, é decisão explícita do dono do projeto — perguntar antes de assumir qualquer um dos dois caminhos.

## Invariantes do envio (valem para o cron uazapi, o envio imediato e o "Forçar envio")

Todos vieram de bugs reais encontrados na auditoria de 2026-09-19 — cada um tem teste em `tests/` que falha se voltar.

1. **Nunca reivindicar (`processando`) antes de saber que dá para enviar.** `enviarWhatsAppImediato` (`lib/whatsapp/enviar-imediato.ts`) só reivindica se `conexoes.status = 'conectado'` — pedido do dono do projeto (2026-09-21): confirmação de pagamento, cobrança manual e boas-vindas saem NA HORA, sem esperar o cron nem respeitar horário de funcionamento (`pagamento_confirmado`/`boasvindas`/`manual` já ignoram a janela em todo o resto do sistema). Desconectada, a notificação fica em `fila` para o cron. Reivindicar e desistir deixava confirmação de pagamento e "Cobrar agora" presos para sempre (o cron só lê `fila`).
