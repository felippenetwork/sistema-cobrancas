...
- **Duas credenciais uazapi diferentes, não confundir.** `UAZAPI_ADMIN_TOKEN` (env var, uma só para toda a plataforma) autentica só a API de ADMINISTRAÇÃO (`/instance/all` — listar/criar/gerenciar instâncias). Quem autentica o ENVIO em si (`/send/text`, `/send/presence`) é o token DA PRÓPRIA INSTÂNCIA, gerado quando ela conecta e salvo em `conexoes.uazapi_instance_token` — nunca precisa do admin token. **Qualquer código que decide "posso enviar para esta conta" deve ler `conexoes.status`/`uazapi_instance_token` do banco (a mesma fonte que o cron e a tela `/conexao` usam), nunca fazer uma checagem ao vivo contra `/instance/all`** — isso criaria uma dependência desnecessária do admin token no caminho crítico de envio. Achado real (2026-09-19, OPS-N3): `UAZAPI_ADMIN_TOKEN` ficou vazio em produção (confirmado com `vercel env pull` + teste direto: `401 Missing AdminToken Header`) e isso fazia "Forçar" reportar "desconectado" com o WhatsApp genuinamente conectado — os lembretes automáticos do cron não foram afetados porque o envio de verdade nunca dependeu desse token, só a sincronização de status (`sincronizarConexoes`) dependia, e vinha falhando calada (capturada pelo try/catch do handler). O admin token continua necessário só para essa sincronização automática (`sincronizarConexoes`, cron `whatsapp-uazapi`) — sem ele, `conexoes.status` fica congelado no último valor conhecido e o banner "WhatsApp desconectado" da Dashboard para de refletir desconexões reais.
