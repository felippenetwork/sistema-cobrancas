...
1. **Nunca reivindicar (`processando`) antes de saber que dá para enviar por aquele caminho.** `enviarWhatsAppImediato` (`lib/whatsapp/enviar-imediato.ts`) tenta Meta primeiro, senão uazapi (se `conexoes.status = 'conectado'`) — pedido do dono do projeto (2026-09-21): confirmação de pagamento e boas-vindas saem NA HORA nos dois canais, sem esperar o cron nem respeitar horário de funcionamento (`pagamento_confirmado`/`boasvindas`/`manual` já ignoram a janela em todo o resto do sistema). Sem nenhum canal disponível, a notificação fica em `fila` para o cron correspondente. Reivindicar e desistir deixava confirmação de pagamento e "Cobrar agora" presos para sempre (o cron só lê `fila`).
